...
namespace _4RTools.Model
{
    public class AutoBuffSkill : Action
    {
        ...
        public _4RThread AutoBuffThread(Client c)
        {
            _4RThread autobuffItemThread = new _4RThread(_ =>
            {
                try
                {
                    ...
                    // === VERIFICAÇÃO SIMPLIFICADA: Apenas prevenir se estiver morto (HP = 0) ===
                    uint currentHp = c.ReadCurrentHp();
                    uint maxHp = c.ReadMaxHp();

                    // Se HP for 0, o personagem está morto - não tentar buffar
                    ...
                    // Verificar se deve pausar autobuff quando montado
                    bool hasMountStatus = c.HasMountStatus();
                    bool stopBuffsOnRein = ProfileSingleton.GetCurrent().UserPreferences.stopBuffsRein;

                    bool canAutobuff = !hasAntiBot
                        && !(hasOpenChat && stopOpenChat)
                        && !(stopBuffsCity && isInCityList)
                        && !(stopBuffsOnRein && hasMountStatus);
                    ...
                    if (canAutobuff)
                    {
                        ...
                        if (!buffs.Contains(EffectStatusIDs.ANTI_BOT) &&
                            (!c.ReadOpenChat() || !ProfileSingleton.GetCurrent().UserPreferences.stopWithChat))
                        {
                            if (!buffs.Contains(EffectStatusIDs.RIDDING) || !ProfileSingleton.GetCurrent().UserPreferences.stopBuffsRein)
                            {
                                // === LIMITADOR: Usar apenas 1 buff por ciclo para evitar spam ===
                                int buffsUsedThisCycle = 0;
                                const int MAX_BUFFS_PER_CYCLE = 1;

                                foreach (var item in bmClone)
                                {
                                    // Parar se já usou o máximo de buffs neste ciclo
                                    if (buffsUsedThisCycle >= MAX_BUFFS_PER_CYCLE)
                                        break;

                                    if (foundQuag && (item.Key == EffectStatusIDs.CONCENTRATION ||
                                        item.Key == EffectStatusIDs.INC_AGI ||
                                        item.Key == EffectStatusIDs.TRUESIGHT ||
                                        item.Key == EffectStatusIDs.ADRENALINE ||
                                        item.Key == EffectStatusIDs.SPEARQUICKEN ||
                                        item.Key == EffectStatusIDs.ONEHANDQUICKEN ||
                                        item.Key == EffectStatusIDs.WINDWALK ||
                                        item.Key == EffectStatusIDs.TWOHANDQUICKEN))
                                    {
                                        break;
                                    }
                                    else if (foundDecreaseAgi && (item.Key == EffectStatusIDs.TWOHANDQUICKEN ||
                                        item.Key == EffectStatusIDs.ADRENALINE ||
                                        item.Key == EffectStatusIDs.ADRENALINE2 ||
                                        item.Key == EffectStatusIDs.ONEHANDQUICKEN ||
                                        item.Key == EffectStatusIDs.SPEARQUICKEN))
                                    {
                                        break;
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"Erro no AutoBuffSkill: {ex.Message}");
                }
        ...
        // Método adicionado para remover a associação de tecla de um buff
